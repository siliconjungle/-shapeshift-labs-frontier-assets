# @shapeshift-labs/frontier-assets

Serializable asset and content provenance graphs for Frontier apps.

`frontier-assets` makes content pipeline state explicit: source files, generated variants, thumbnails, LOD chunks, shaders, materials, transforms, hashes, owners, runtime consumers, build tasks, scenes, routes, tests, registry graph output, impact plans, and proof hashes.
...
```sh
npm install @shapeshift-labs/frontier-assets
```
...
import {
  ...
  planAssetReview,
  traceAssetImpact
} from '@shapeshift-labs/frontier-assets';

const catalog = compileAssetCatalog(createAssetCatalog({
  id: 'game.content',
  resources: [{
...
const impact = traceAssetImpact(catalog, {
  changedResources: ['shader:foliage']
});

const review = planAssetReview(catalog, {
  changedResources: ['shader:foliage']
});
```
...
- `createAssetCatalog`, `defineAsset`, `compileAssetCatalog`, `validateAssetCatalog`, and `queryAssetCatalog` describe asset/content resources.
- `traceAssetImpact` answers which assets, outputs, variants, scenes, routes, tasks, tests, and consumers are affected by changed files, hashes, outputs, or resources.
- `planAssetReview` converts impact into review items with owners and related resources.
- `createAssetRegistryGraph` exposes content resources as Frontier registry entries and edges.
- `encodeAssetJsonl`, `decodeAssetJsonl`, `redactAssetValue`, and `createAssetProof` support evidence bundles, trace attachments, and replay.

## Manifest Boundary

`@shapeshift-labs/frontier-manifest` records static application features, source files, assets, resources, and build tasks. It is the application map.

`@shapeshift-labs/frontier-assets` owns detailed content provenance: generated variants, thumbnails, LOD chunks, shader/material dependencies, content hashes, source-to-output transforms, runtime consumers, and review impact. Manifest can point at asset resources; it should not need to become an asset build graph.

`@shapeshift-labs/frontier-worker`, `@shapeshift-labs/frontier-scene`, `@shapeshift-labs/frontier-route`, `@shapeshift-labs/frontier-view`, and `@shapeshift-labs/frontier-lod` remain structural consumers. This package records that a task generated an asset, a scene uses it, or a route should review it, without importing those runtime packages.
...
The benchmark writes `benchmarks/results/frontier-assets-package-bench-latest.json` when run from the monorepo. These are Frontier-only package measurements for catalog creation, compilation, validation, query, impact tracing, review planning, registry graph output, JSONL, and proof hashing.
